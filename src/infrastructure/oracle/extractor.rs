...
impl ParquetRowWriter {
    fn new(
        ...
        let arrow_fields: Vec<_> = col_infos
            .iter()
            .map(|c| {
                let dt = mapping::map_oracle_to_arrow(
                    c.oracle_type(),
                    Some(c.oracle_type().to_string().as_str()),
                );
        ...
        let props = WriterProperties::builder()
            .set_compression(Self::map_compression(compression))
            .build();
...
impl ExtractionPort for Extractor {
    ...
    fn export_task(&self, task: ExportTask, metadata: &TableMetadata) -> Result<TaskResult> {
        ...
        let mut writer: Box<dyn RowWriter> = match task.file_format {
            FileFormat::Csv => Box::new(CsvRowWriter::new(
                &task.output_file,
                self.field_delimiter,
                rows.column_info(),
            )?),
            FileFormat::Parquet => Box::new(ParquetRowWriter::new(
                &task.output_file,
                &task.parquet_compression,
                task.parquet_batch_size.unwrap_or(10000),
                rows.column_info(),
            )?),
        };
...
impl ArrowPush for TimestampPush {
    fn push(&mut self, row: &Row, i: usize, _ex: &Extractor) -> Result<u64> {
        let v: Option<Timestamp> = row
            ...
            .map_err(|e| ExportError::OracleError(e.to_string()))?;
        if let Some(ts) = v {
            if let Some(dt) = chrono::NaiveDate::from_ymd_opt(ts.year(), ts.month(), ts.day())
                .and_then(|d| {
                    d.and_hms_micro_opt(ts.hour(), ts.minute(), ts.second(), ts.nanosecond() / 1000)
                })
            {
